...
--
-- Host: 127.0.0.1
-- Tempo de geração: 09-Nov-2022 às 04:21
...
DECLARE p_check_admission_module_qtd integer; #== Checa qtd de módulos de admissão
DECLARE p_check_followup_module_qtd integer; #== Checa qtd de módulos de Acompanhamento
DECLARE p_check_discharged_module_qtd integer; #== Checa qtd de módulos de Desfecho
...
sp:BEGIN 
  ...
  if (p_check_discharged_module_qtd <> 1) then
	SET p_msg_retorno =  CONCAT('A pesquisa tem ',p_check_discharged_module_qtd,' módulos de Desfecho. Deveria ter 1 (um).');
    leave sp;
  end if;  
  
  SET p_check_admission_module_questions_qtd = (SELECT count(questionID) FROM tb_questiongroupform where crfFormsID = (Select crfFormsID from tb_crfforms where questionnaireID = p_questionnaireID and description = 'Admission Form') );
  
  if (p_check_admission_module_questions_qtd = 0) then
	SET p_msg_retorno =  CONCAT('A pesquisa não pode ser publicada porque há ',p_check_admission_module_questions_qtd,' questões no módulos de Admissão.');
    leave sp;
  end if;  
  
    SET p_check_followup_module_questions_qtd = (SELECT count(questionID) FROM tb_questiongroupform where crfFormsID = (Select crfFormsID from tb_crfforms where questionnaireID = p_questionnaireID and description = 'Follow-up'));
  
  if (p_check_followup_module_questions_qtd = 0) then
	SET p_msg_retorno =  CONCAT('A pesquisa não pode ser publicada porque há ',p_check_followup_module_questions_qtd,' questões no módulos de Acompanhamento.');
    leave sp;
  end if;  
  
  SET p_check_discharged_module_questions_qtd = (SELECT count(questionID) FROM tb_questiongroupform where crfFormsID = (Select crfFormsID from tb_crfforms where questionnaireID = p_questionnaireID and description = 'Discharge/death form'));
  
  if (p_check_discharged_module_questions_qtd = 0) then
	SET p_msg_retorno =  CONCAT('A pesquisa não pode ser publicada porque há  ',p_check_discharged_module_questions_qtd,' questões no módulos de Desfecho.');
    leave sp;
  end if;  
...
#========================================================================================================================
 ...
 select crfformsid as modId, questionid as qstId, questionorder as qstOrder, qstgroupid as qstGroupId,
	      IF(LENGTH(translate('pt-br', questionGroup))>0, translate('pt-br', questionGroup), questionGroup) as dsc_qst_grp,
		  IF(LENGTH(translate('pt-br', question))>0, translate('pt-br', question), question) as dsc_qst,
	            ...
	            (select  GROUP_CONCAT(IF(LENGTH(translate('pt-br',tlv.description))>0, translate('pt-br',tlv.description), tlv.description)) as listofvalue from tb_listofvalues tlv
	            ...
	            (select  GROUP_CONCAT(convert(tlv1.listOfValuesID, char)) as listofvalueID from tb_listofvalues tlv1
	             ...
	             IF(LENGTH(translate('pt-br', questaosubordinada_a))>0, translate('pt-br', questaosubordinada_a), questaosubordinada_a) as sub_qst,
...
sp:BEGIN 
   ...
   START TRANSACTION;
    ...
    set last_id = (Select MAX(crfFormsID) + 1 from tb_crfforms);
    INSERT INTO tb_crfforms (
			crfformsID, questionnaireID, description, crfformsStatusID, lastModification,creationDate )
            values (last_id, p_questionnaireID, translate('eng', p_moduleDescription), p_moduleStatusID, NOW() ,NOW());
...
(5, 'teste', 2, 1, 0, '0.0', '2022-11-05 18:55:29', '2022-11-05 18:55:29');
...
--
ALTER TABLE `tb_questionnaire`
  MODIFY `questionnaireID` int(255) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;
